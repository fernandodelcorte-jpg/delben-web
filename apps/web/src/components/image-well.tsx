...
export function ImageWell({
  ...
  return (
    ...
    >
      ...
      <span className="pointer-events-none absolute left-4 top-4 h-3.5 w-3.5 border-l border-t border-current opacity-40" aria-hidden />
      <span className="pointer-events-none absolute bottom-4 right-4 h-3.5 w-3.5 border-b border-r border-current opacity-40" aria-hidden />

      {/* Etiqueta del marcador, en esquina para no estorbar al texto */}
      <span className="absolute right-4 top-4 z-10 font-sans text-[0.65rem] uppercase tracking-[0.16em] opacity-70">
        [{label}]
      </span>
